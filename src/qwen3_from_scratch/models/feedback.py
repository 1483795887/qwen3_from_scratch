...
class MyFeedback(PythonFeedback):
    def __init__(self, *args, **kwargs):
        ...
        merged_weight = torch.concat(
            [self.up_proj.weight, self.gate_proj.weight], dim=0
        )
    ...
    def forward(self, x, residual=None):
        if x.is_cuda:
            from qwen3_from_scratch.kernels.triton.feedback import (
                simple_swiglu,
            )
...
class MoE(nn.Module):
    def __init__(self, config: ModelConfig, name: str, **kwargs):
        super().__init__()
        self.name = name
        assert config.num_experts > 0 and config.num_experts_per_token > 0, (
            "Moe需要设置 num_experts 和 num_experts_per_token"
        )
        ...
        self.experts = nn.ModuleList(
            [
                PythonFeedback(config, name + f".experts.{i}", **kwargs)
    ...
    def forward(self, x, residual=None):
        ...
        topk_weight, topk_idx = torch.topk(
            scores, k=self.num_experts_per_token
        )
        ...
        for i, expert in enumerate(self.experts):
            ...
            result.index_add_(
                0,
                token_idx,
                (expert(hidden_states[token_idx]) * weight).to(result.dtype),
            )
