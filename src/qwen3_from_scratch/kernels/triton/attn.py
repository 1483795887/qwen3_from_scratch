...
from qwen3_from_scratch.kernels.triton.gemm import gemm_without_c
...
def softmax(
    ...
    for row_idx in tl.range(row_start, M, row_step):
        ...
        row = tl.load(
            input_ptr,
            mask=mask & ((not is_causal) | (col_offsets <= row_idx)),
            other=-float("inf"),
        )
        max_val = tl.max(row)
        row = row - max_val
...
def fused_attention_intr(
    ...
    if STAGE == 1:
        ...
        lo, hi = (
            min(start_m * BLOCK_SIZE_M, N_KEY),
            min((start_m + 1) * BLOCK_SIZE_M, N_KEY),
        )
    else:
        lo, hi = 0, N_KEY
...
def flash_attention(
    ...
    dtype = (
        ...
        else (tl.float16 if Q.dtype == torch.float16 else tl.bfloat16)
    )
...
def cpu_forward(q, k, v, head_dim, is_causal: bool = True):
    ...
    if seq_len_q > 1 and is_causal:
        ...
        scores = scores.masked_fill(
            ~causal_mask.unsqueeze(0).unsqueeze(0).unsqueeze(0), float("-inf")
        )
...
def main():
    ...
    for dtype in [
        ...
        k = torch.rand(
            B, N, H // groups, D, dtype=dtype, device="cuda"
        ).transpose(1, 2)
        v = torch.rand(
            B, N, H // groups, D, dtype=dtype, device="cuda"
        ).transpose(1, 2)
