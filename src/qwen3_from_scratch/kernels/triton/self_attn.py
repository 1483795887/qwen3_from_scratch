...
def fused_qk_norm_rope(
    ...
    fused_qk_norm_rope_kernel[grid](
        qkv, gamma, cos_embed, sin_embed, N, H_q, H_q + 2 * H_kv, HEAD_DIM, eps
    )
...
def test_fused_norm_rope():
    ...
    with torch.no_grad():
        ref_q, ref_k, _ = qkv.view(B, N, -1, HEAD_DIM).split(
            [H_q, H_q // groups, H_q // groups], dim=-2
        )
        ref_q, ref_k = ref(ref_q, ref_k, cos_embed, sin_embed)

        fused_qk_norm_rope(qkv, gamma, cos_embed, sin_embed, HEAD_DIM, groups)
        target_q, target_k, _ = qkv.view(B, N, -1, HEAD_DIM).split(
            [H_q, H_q // groups, H_q // groups], dim=-2
        )
