...
@triton.jit
def gemm_kernel_core(a_ptr, b_ptr, acc, BLOCK_SIZE_K, K):
...
def gemm(
    ...
    assert a.shape[0] == b.shape[0] == c.shape[0] == d.shape[0], (
        f"Batch size mismatch: {a.shape[0]} vs {b.shape[0]} vs {c.shape[0]} vs {d.shape[0]}"
    )
    B, Ha, M, K = a.shape
    assert K == b.shape[-2], (
        f"Inner matrix dimension mismatch: {K} vs {b.shape[-2]}"
    )
    _, Hb, _, N = b.shape
    assert c.shape[-2:] == (M, N) == d.shape[-2:], (
        f"Output shape mismatch: {c.shape[-2:]} vs {(M, N)}"
    )
    assert Ha == c.shape[1] == d.shape[1], (
        f"Feature dimension mismatch: {c.shape[1]} vs {d.shape[1]}"
    )
    assert Ha % Hb == 0
...
def linear(
    ...
    D1, D = w.shape
    assert D == x.shape[-1], (
        f"x feature dim mismatch: expected {D}, got {x.shape[-1]}"
    )
...
def grouped_gemm(
    ...
    assert (
        ...
        f"grouped size should be same: len(a) {len(a_tensors)} vs len(b) {len(b_tensors)} vs len(c) {len(c_tensors)} vs len(alpha) {len(alpha)} vs len(beta) {len(beta)}"
    )
    assert len(a_tensors) > 0
    device = a_tensors[0].device
    ab_dtype = (
        tl.float32 if a_tensors[0].dtype == torch.float32 else tl.float16
    )
    cd_dtype = (
        tl.float32 if d_tensors[0].dtype == torch.float32 else tl.float16
    )
    NUM_SMS = 84
...
if __name__ == "__main__":
    ...
    device = "cuda"
    a_tensors = [
        torch.rand(M, K, dtype=torch.float32, device=device) * 10
        for _ in range(groups)
    ]
    b_tensors = [
        torch.rand(K, N, dtype=torch.float32, device=device) * 10
        for _ in range(groups)
    ]
