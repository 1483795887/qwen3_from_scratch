...
template <typename T>
void rms_norm_forward_(const T* x,
                       T* output,
                       const T* gamma,
                       const int seqLen,
                       const int hiddenDim,
...
torch::Tensor rms_norm_forward_cpu(const torch::Tensor& x, const torch::Tensor& gamma, const float eps = 1e-6f) {
    ...
    if (x.dtype() == torch::kF16) {
        rms_norm_forward_<at::Half>(x.data_ptr<at::Half>(),
...
// -------------------------- 设备自动选择的封装函数 --------------------------
torch::Tensor rms_norm_forward(const torch::Tensor& x, const torch::Tensor& gamma, const float eps) {
