...
from qwen3_from_scratch.kernels.triton.fused.fused_rms_norm_linear import (
...
class SimpleFusedRmsNormLinear(nn.Module):
    def __init__(
        ...
        self.linear = nn.Linear(
            input_features, output_features, bias=bias, dtype=dtype
        )
...
def test_fused_rms_norm_linear_1d(dtype, bias):
    ...
    simple_fused_rms_norm = SimpleFusedRmsNormLinear(
        D, D1, dtype=dtype, bias=bias
    ).cuda()
