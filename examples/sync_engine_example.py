...
def main():
    ...
    print("\n=== 单请求非流式 ===")
    text = engine.generate("你好", max_new_tokens=50)
    print(text)
    ...
    texts, batch = engine.batch_generate(prompts, max_new_tokens=50)
    for t in texts:
        print(f"- {t}")
    print(
    ...
    for i, m in enumerate(batch.per_request):
        print(
            f"  req{i}: TTFT {m.ttft:.4f}s, TPS {m.tps:.2f}, tokens {m.token_count}"
        )
