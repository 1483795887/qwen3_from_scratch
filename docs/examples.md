...
| [llm_example.py](../examples/llm_example.py) | 基于 `LLM` 类的进程内同步推理示例，支持 `generate`、`batch_generate` 与 `enable_thinking` |
| [async_llm_example.py](../examples/async_llm_example.py) | 基于 `AsyncLLM` 的推理引擎示例，支持流式生成与多模型批量配置，详见 [启动指南](quickstart.md) |
...
默认监听 `0.0.0.0:8887`。默认使用 `FakeEngine`（返回固定的 "hello world"），用于在无模型环境下验证服务链路；加上 `--use_real_model` 后改用真实的 `AsyncLLM` 推理：
...
- 基于配置文件的示例（`run_from_config.py`、`async_llm_example.py`、`llm_example.py`）将 `examples/configs/*.yaml` 中的 `path` 改为自己的模型下载路径
